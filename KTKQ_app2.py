...
class mainWin(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def on_cmd_pltkq_clicked(self):
        ...
        for pd in self.pd:   
            kqs = [kq( j, pd, self.aea0, self.z) for j in js]
            kts = [kt( j, pd, self.aea0, self.z) for j in js]
                
            tab = np.transpose(np.vstack((js,kqs,kts)))
            dat =  dat+ tabulate(tab,headers = ['j' ,'kq', 'kt'],tablefmt = "plain", stralign = 'right') +"\n"
            self.kqvals = np.array([kq(v,pd, self.aea0,self.z) for v in self.jvals])
            self.ktvals = np.array([kt(v,pd, self.aea0,self.z) for v in self.jvals])
