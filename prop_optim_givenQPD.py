...
def getIntersect(x1,y1, x2, y2, deg):
    
    fit1 = np.polyfit(x1,y1,deg)
    fit2  = np.polyfit(x2, y2, deg)
    eqn = fit1 - fit2
    roots = np.roots(eqn)
    return roots
...
#    print '\n kq for pd / j\n'+ tabulate(tabkq, headers= list(tabkt.dtype.names), tablefmt='grid')
    # Optimium n from D
    v_ship = 16 * 0.5144 # velocity in metre per seconds
    w = 0.3 # wake
    rho = 1.025 # t/m3
    ...
    row = 0 
    for j in pdnams:
        curint = getIntersect(jvals, tabkq[j], jvals, kq_j5, 5)
        print curint
        valans = filter(lambda x : x > 0 and x <  max(jvals), curint)
        cj = valans[-1].real
        print cj
        ckt = kt(valans[-1],float(j), aea0, z)
        ckq = kq(valans[-1],float(j), aea0, z)
        eta0 = (cj * ckt)/ (ckq *2 * 3.14)
        optivals[row] = np.array([float(j), cj, ckt , ckq,eta0])
        
        plotInter(ax,  valans[-1], kq(valans[-1],float(j),aea0,z))
        row +=1 
